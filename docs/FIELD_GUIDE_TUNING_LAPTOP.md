...
### Option A: PEAK-System PCAN-USB (Recommended)
...
3. Plug in your PCAN-USB dongle. The LED should illuminate solid red, then blink green upon bus detection.
4. Launch using `2_RUN_LIVE_PCAN.bat`.

### Option B: CANable / CANable Pro (SLCAN Mode)
...
```
 ...
 └──────────┬───────────┘
            ▼
 ┌──────────────────────┐
 │  Step 2: Start Log   │  Run '2_RUN_LIVE_PCAN.bat' or '3_RUN_LIVE_CANABLE_SLCAN.bat'
...
| **PCANBasic library not found** | Missing Peak drivers | Run PEAK-System driver installer on laptop. |
